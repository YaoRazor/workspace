...
class MyMMU
{
  ...
  void getString(string input)
    ...
    *pointer_input=temp;	
		...
		{
			instruction[i].op=atoi((*pointer_input)[2*i].c_str());
			//cout<<"instruction["<<i<<"].op="<<instruction[i].op<<endl;
			instruction[i].vp_num=atoi((*pointer_input)[2*i+1].c_str());
			//cout<<"instruction["<<i<<"].vp_num="<<instruction[i].vp_num<<endl;
		}
